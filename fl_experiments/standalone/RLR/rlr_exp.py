...
from fl_api.standalone.RLR.rlr_api import RLRAPI
from fl_api.data_preprocessing.data import get_datasets, distribute_data_dirichlet, distribute_data, DatasetSplit, poison_dataset
import torch
...
def add_args(parser):
    ...
    parser.add_argument('--method', type=str, default="rlr",
                        help="num of workers for multithreading")
...
def load_data(args):
    ...
    poisoned_val_loader = DataLoader(poisoned_val_set, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers,
                                     pin_memory=False)
    ...
    if args.dataset != "tinyimagenet":
        ...
        poisoned_val_set_only_x = DatasetSplit(copy.deepcopy(val_dataset), idxs, runtime_poison=True, args=args, modify_label =False)
...
if __name__ == "__main__":
    ...
    for poison_frac_ in poison_frac_list:
        ...
        print("{}".format(args))
        mnt_flAPI = RLRAPI(dataset, device, args, logger)
        mnt_flAPI.train()
