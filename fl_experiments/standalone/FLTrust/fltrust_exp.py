...
from fl_api.standalone.FLTrust.fltrust_api import FLTrustAPI
...
if __name__ == "__main__":
    ...
    parser = add_args(argparse.ArgumentParser(description='Trimmed mean'))
    args = parser.parse_args()

    device = torch.device("cuda:" + str(args.gpu) if torch.cuda.is_available() else "cpu")

    log_path = os.path.join('./logs/', args.dataset)
    log_name = "FLTrust_AckRatio{}_{}_method{}_data{}_alpha{}_epoch{}_inject{}_agg{}_nonIID{}_theta{}_attack{}_topk{}".format(
    ...
    print("{}".format(args))
    mnt_flAPI = FLTrustAPI(dataset, device, args, logger)
    mnt_flAPI.train()
