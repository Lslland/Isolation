...
from fl_api.utils.general import parameters_to_vector, visual_loss_distribution, smooth_grad_CAM, \
    compute_cosine_similarity, write_data_to_csv, compute_l2norm_feature, visual_neuron_activation, computing_coordinate_repeatability
...
class IsolationAPI:
    def __init__(self, dataset, device, args, logger):
        ...
        self.train_dataset, self.poisoned_val_loader, self.poisoned_val_only_x_loader, self.user_groups, self.val_loader, self.poisoned_clean_val_loader = dataset
    ...
    def train(self):
        ...
        for round_idx in range(1, self.args.comm_round + 1):
            ...
            for client_id in client_indexes:
                ...
                topk_mask, client_params, client_grads = client.train(copy.deepcopy(w_per_models[client_id]), round_idx,
                                                                      self.criterion, neurotoxin_mask=neurotoxin_mask)
    ...
    def test(self, global_model, round_idx):
        # feature_l2_norm = compute_l2norm_feature(global_model, self.model, self.poisoned_clean_val_loader, self.poisoned_val_loader, self.device)
        # print('feature_l2_norm: ', feature_l2_norm)

        val_loss, (val_acc, val_per_class_acc), _, val_samples_losses = self.get_loss_n_accuracy(global_model,
                                                                                                 self.criterion,
                                                                                                 self.val_loader,
                                                                                                 self.args, round_idx,
                                                                                                 self.args.n_classes,
        ...
        poison_loss, (asr, _), fail_samples, poison_samples_losses = self.get_loss_n_accuracy(global_model,
                                                                                              self.criterion,
                                                                                              self.poisoned_val_loader,
                                                                                              self.args, round_idx,
                                                                                              self.args.n_classes,
                                                                                              name=None)
        ...
        # names, y_clean, y_poison, clean_loss_avg, poison_loss_avg = visual_loss_distribution(val_samples_losses,
        #                                                                                      poison_samples_losses,
        #                                                                                      title)
        ...
        # return names, y_clean, y_poison, clean_loss_avg, poison_loss_avg
    ...
    def get_loss_n_accuracy(self, w_global, criterion, data_loader, args, round_idx, num_classes=10, name=None):
        ...
        if name is not None and self.args.grad_cam:
